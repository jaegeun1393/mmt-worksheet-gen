...
function send_section1() { //when request wkst form
	var accept = 0;
	var seed = "";
	...
	if(min >= max) {
		...
		alert("The minimum value should be at leat 100.");
		accept = 1;	
	}
	else if(max > 900) {
	...
	if(accept == 0) {
		...
		var preseed = document.getElementById("seed").innerText.replace(/[\n\r]+|[\s]{2,}/g, ' ').trim(); //Front seed
	}
	seed = preseed + "-" + nump + "&&";
